# -*- coding: utf-8 -*-
"""
Created on Tue Aug  3 18:36:08 2021
...
class Blockchain:
    ...
    def proof_to_work(self, previous_proof):
        check_proof = False
        new_proof = 1;
...
# Creating address for node at port 5000
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Chotu', amount = 1)
...
def connect_node():
    json = request.get_json()
    nodes = json.get('nodes') #address of nodes
    if nodes is None:
        return 'No node', 400
    for node in nodes:
        blockchain.add_node(node)
    response = {'message' : 'All nodes are now connected. The Chandcoin blockchain now contains the : ',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message' : 'The nodes had different chain so chain was replaced by longest one.',
